...
public class SpheresScene : IScene
{
    public HittableGroup Render()
    {
        ...
        for (var j = -11; j < 11; j++)
        {
            var materialProbability = Random.Shared.NextSingle();
            ...
            {
                IMaterial material;
                var albedo = ColorHelper.FromRandom();
                ...
                {
                    var fuzz = Random.Shared.NextSingle() * 0.5f;
                ...
                var center2 = center + new Vector3(0f, 0.5f * Random.Shared.NextSingle(), 0f);
