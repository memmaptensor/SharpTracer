...
public class MetalMaterial : IMaterial
{
    ...
    {
        var reflected = Vector3.Reflect(Vector3.Normalize(ray.Direction), hit.Normals);
        outRay = new Ray(hit.Position, reflected + Fuzziness * Sphere.RandomPointInSphere(), ray.Time);
        attenuation = Albedo;
